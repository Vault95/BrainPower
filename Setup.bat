:: The Boys Fika Co-Op Server Pack - Client Setup Batch Script
:: Author: Codsworth
...
if "%localversion%"=="%publicversion%" echo "Installer up to date... proceeding with setup"
if %filename%==%filename2% powershell -ExecutionPolicy Bypass -File Setup.ps1
...
if NOT "%localversion%"=="%publicversion%" -Command "Invoke-WebRequest 'https://www.dropbox.com/scl/fi/3w7l7kgrkbwv7muj1jdsq/Setup.ps1?rlkey=pxhb32hnrba94t5fnnqvtbkd6&st=2nvqo2a8&dl=1' -OutFile %~dp0Setup.ps1"
if NOT "%localversion%"=="%publicversion%" -Command "Invoke-WebRequest 'https://www.dropbox.com/scl/fi/ennk9wdqh8yk0vathqngg/Update.ps1?rlkey=g9e6xzhauqvhjsk64e49rc5ef&st=ui6vkiyo&dl=1' -OutFile %~dp0Update.ps1"
if NOT "%localversion%"=="%publicversion%" -Command "Invoke-WebRequest 'https://www.dropbox.com/scl/fi/spl8kfpgvvuuvq1qhjl2z/version.txt?rlkey=lzzfb3cf67zqatzn5978gxpfy&st=e4g5vih4&dl=1' -OutFile %~dp0version.txt"
